  ...
  if (isMinimized) {
    return (
      <button 
        ...
        className="absolute top-6 left-8 w-12 h-12 bg-black/60 backdrop-blur-md border border-white/10 rounded-full flex items-center justify-center z-50 hover:bg-white/10 transition-colors shadow-lg shadow-purple-900/20"
        title="Show Music Player"
      >
        <Music size={20} className={`text-white ${isPlaying ? 'animate-pulse' : ''}`} />
      </button>
    );
  }

  return (
    <div className="absolute top-6 left-8 w-80 bg-black/60 backdrop-blur-md border border-white/10 rounded-2xl p-4 z-50 shadow-2xl shadow-purple-900/20 transition-all">
      {/* 修改头部布局，加入最小化按钮 */}
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center gap-4">
          ...
          <div className="overflow-hidden w-48"> {/* 限制宽度防止文字溢出 */}
